...
function my_acf_google_map_api( $api ){
	...
	$api['key'] = 'AIzaSyAcGBUQQTDaszOJpko3gijEOx-TcP8bRwo';
...
function gnc_currency_settings_init(  ) {
	...
	add_settings_section(
      ...
      __( 'USD To Colon Exchange Rate <h2>Pasos: </h2> <br> 1 guardar los cambios <br> 
      2 Ir a la pagina de refrescar precios <a href="https://gnc.cr/run-dollar-product-price-update/"   target="_blank">Refrescar todos los precios</a>  ', 'gnc_storefront' ),
...
//add_action( 'save_post', 'woocommerce_Save_product_simple' , 10, 1 ); 
...
function save_variation_settings_fields( $variation_id, $loop ) {
	$text_field = $_POST['_dolar_price_field'][ $loop ];

	if ( ! empty( $text_field ) ) {
			update_post_meta( $variation_id, '_dolar_price_field', esc_attr( $text_field ));
	}
}

function load_variation_settings_fields( $variation ) {     
    //$variation['_dolar_price_field'] = get_post_meta( $variation[ '_dolar_price_field' ], '_dolar_price_field', true );
...
function variation_settings_fields1( $loop, $variation_data, $variation ) {
	...
	echo '<p class="form-row col-3 firstCol  divchangeDolar"> <a href="/wp-admin/options-general.php?page=gnc_storefront" target="_blank">Modificar tipo de cambio</a></p>';
		...
		woocommerce_wp_text_input(
			// Number Field Dolares regular price
				array(
						'id'            => "_dolar_price_field{$loop}",
						'value'         => get_post_meta( $variation->ID, "_dolar_price_field0", true ),
						'label'         => __( 'Precio en dolares', 'woocommerce' ),
						'desc_tip'      => true,
						'description'   => __( 'ponga el precio en dolares del producto', 'woocommerce' ),
						'wrapper_class' => 'divRegularPrice',
	...
	// Number Field Dolares regular price
			array(
					...
					'value'         => get_post_meta( $variation->ID, "_dolar_price_field{$loop}", true ),
					'label'         => __( 'Promocion en dolares', 'woocommerce' ),
					'desc_tip'      => true,
					'description'   => __( 'ponga la Promocion en dolares del producto', 'woocommerce' ),
					'wrapper_class' => 'divSalePrice',
...
function WC_Save_product($post_id) {
	...
	if( $type == 'simple' ){ 
		...
		// Dolares presente
		if( ! empty( $dolar_price_field ) ) {
			$regular_price = $dolar_price_field * $rate;
		...
		if( ! empty( $dolarSale_price_field ) &&  ! empty( $dolar_price_field )) {
			...
			$promo				 =  $dolarSale_price_field * $rate;
			$sales_price	 = $promo;
			$sales_price	 = round($sales_price);
	...
	if( $type == 'variable' ){ 
		...
		$dolar_price_field 		 = isset( $_POST['_dolar_price_field'] ) ? $_POST['_dolar_price_field'] : '';
		$regularPrice 		 		 = isset( $_POST['variable_regular_price_'] ) ? $_POST['variable_regular_price_'] : '';

		$meta = get_post_meta($post_id, '', true);
		//_dolar_price_field 


		die(var_dump( $regularPrice));
// Dolares presente
    if ( ! empty( $dolar_price_var ) ) {
			$regular_price = $dolar_price_field * $rate;
			$regular_price = round($regular_price);
	
			// update_post_meta( $variation_id, 'variable_regular_price', esc_attr( 11 ));
			// update_post_meta( $variation_id, '_dolar_price_field', esc_attr( '22' ));
		}
		
		

// Number Field Dolares regular price
foreach( $dolar_price_field as $dolarFields){

	// check if the product has Dollar value
						
		if (!empty( $dolarFields )){
								$regular_price = $dolarFields * $rate;
								$regular_price = round($regular_price);
								update_post_meta($variation_id, '_regular_price', '12345');
	...
	// }
		...
		////////////  2. Save
		// update_post_meta($post_id, 'variable_regular_price', 						$sales_price);


	} // end variable
		
}// WC_Save_product ends
