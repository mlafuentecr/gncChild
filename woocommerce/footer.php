...
///Check Cedula
	 if (cedula.length < 1) {
				 jQuery('#cedula').css({'background': 'yellow'});
				 jQuery('#cedula').after('<span class="error"> Eje: 109070047 </span>');
				 jQuery('#cedula').attr("placeholder", "109070047");
				 jQuery('#cedula').attr("placeholder", "109070047").blur();

	}else if(!numberReg.test(cedula)){
			 jQuery('#cedula').css({'background': '#f4c3c3'});
			 jQuery('#cedula').css({'background': 'yellow'});
				 jQuery('#cedula').after('<span class="error"> Eje: 109070047 </span>');
				 jQuery('#cedula').attr("placeholder", "109070047");
				 jQuery('#cedula').attr("placeholder", "109070047").blur();
	}else{
			jQuery('#cedula').css({'background': '#f2f2f2'});
			jQuery('#cedula').next('span').remove();
	}

///Check Genero
	if (genero.length < 1) {
		error('#genero', idDiv, TextDiv);
				jQuery('#genero').css({'background': '#f4c3c3'});
 }else if(!genReg.test(genero)){
			jQuery('#genero').css({'background': '#f4c3c3'});
		
		//	jQuery('#genero').after('<span class="error"> F o M </span>');

 }else{
	    jQuery('#genero').css({'background': '#f2f2f2'});
			jQuery('#genero').next('span').remove();
 }


 ///Check nacimiento
 	if (nacimiento.length < 1) {
 				jQuery('#nacimiento').css({'background': '#f4c3c3'});
  }else if(!genReg.test(nacimiento)){
 			jQuery('#nacimiento').css({'background': '#f4c3c3'});



  }else{
 	    jQuery('#nacimiento').css({'background': '#f2f2f2'});
 			jQuery('#nacimiento').next('span').remove();
  }
...
function error(whereID, idDiv, TextDiv){
	alert('entre');
	if (jQuery('#errorNacimiento').length < 1) {
		 jQuery(whereID).after('<span  id="errorNacimiento"> DiaMesAño </span>');
	} else {
		jQuery(whereID).text().replace('<span  id="errorNacimiento"> DiaMesAño </span>');
	}
}
